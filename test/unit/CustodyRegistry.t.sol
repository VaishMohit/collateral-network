...
contract CustodyRegistryTest is TestBase {
    ...
    function test_updateFromAttestationRecordsState() public {
        ...
        CustodyRegistry.CustodyState memory cs = custodyRegistry.getCustodyState(C.T_BOND, bankA);
    ...
    function test_encumbranceOnlyCollateralManager() public {
        ...
        custodyRegistry.applyEncumbrance(C.T_BOND, bankA, 100);
    }

    function test_applyAndReleaseEncumbrance() public {
        _attest(C.T_BOND, bankA, 10_000, custodianA, C.PK_CUSTODIAN_A);
        vm.prank(address(collateralManager));
        custodyRegistry.applyEncumbrance(C.T_BOND, bankA, 4_000);
        assertEq(custodyRegistry.availableQuantity(C.T_BOND, bankA), 6_000);
        assertEq(custodyRegistry.getCustodyState(C.T_BOND, bankA).encumberedQuantity, 4_000);
        vm.prank(address(collateralManager));
        custodyRegistry.applyEncumbrance(C.T_BOND, bankA, -4_000);
        assertEq(custodyRegistry.availableQuantity(C.T_BOND, bankA), 10_000);
    }
    ...
    function test_overEncumbranceReverts() public {
        ...
        custodyRegistry.applyEncumbrance(C.T_BOND, bankA, 10_001);
    }
    ...
    function test_revokeOnCustodyDoesNotUnsetState() public {
        ...
        AttestationRegistry.StoredAttestation memory stored = attestationRegistry.getAttestation(
            custodyRegistry.getCustodyState(C.T_BOND, bankA).lastAttestationId
        );
        vm.prank(custodianA);
        attestationRegistry.revokeAttestation(stored.data.attestationId);
        assertEq(custodyRegistry.getCustodyState(C.T_BOND, bankA).totalQuantity, 10_000);
    }
}
