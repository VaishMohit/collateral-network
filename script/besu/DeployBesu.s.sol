...
/**
 * @title DeployBesu
 * @notice Deploys and fully configures the Institutional Collateral Network on
 *         the local Hyperledger Besu QBFT devnet (infra/besu). Identical wiring
 *         to the Anvil Deploy script, but writes the address map to
 *         deployments/besu.json.
 *
 *   The Besu deployment maps the consortium's validator model onto the
 *   protocol's RBAC: each "participating bank" validator node grounds a BANK
 *   role. For this devnet the well-known Anvil development keys are used (and
 *   must be pre-funded on the Besu chain); methods inherit exactly from the
 *   Anvil deployment.
 *
 *   Usage:
 *     docker compose up -d                 # in infra/besu
 *     # fund the deploy accounts on the QBFT chain first
 *     forge script script/besu/DeployBesu.s.sol \
 *       --rpc-url http://127.0.0.1:8545 --broadcast
 */
contract DeployBesu is Script {
    ...
    function run() external {
        ...
        console2.log("=== Institutional Collateral Network deployed on Besu QBFT ===");
        ...
        console2.log("address map: deployments/besu.json");
    }
    ...
    function _writeDeployment() internal {
        ...
        vm.serializeString(root, "network", _networkJson());

        string memory out = vm.serializeString(
            root,
            "note",
            "BESU QBFT DEVNET (chainId 1337, 3 bank validators) - deterministic development keys. Never use in production."
        );
        vm.writeJson(out, "deployments/besu.json");
    }
    ...
    function _networkJson() internal returns (string memory) {
        string memory n = "network";
        vm.serializeString(n, "type", "Hyperledger Besu QBFT consortium (permissioned PoA)");
        vm.serializeString(n, "consensus", "QBFT");
        vm.serializeString(n, "blockperiod", "2s");
        vm.serializeUint(n, "chainId", block.chainid);
        vm.serializeAddress(n, "validator1", 0x9Bba576DfFAE241181E3826d11Ee2d084FD6B045);
        vm.serializeAddress(n, "validator2", 0x1a60a16f94F1C37F8d63289E8b16b684d50A36a8);
        return vm.serializeAddress(n, "validator3", 0x3c3353ecC3c58Df65097050005418357DB35a86d);
    }
