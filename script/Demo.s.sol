...
contract Demo is Script {
    ...
    function run() external {
        ...
        if (doPhase1) {
            ...
            // Phase 4: price drop -> preview -> evaluateAll raises a call ->
            // substitution cures it -> satisfy. Isolated in a helper to keep the
            // many locals out of run()'s stack frame.
        ...
        if (doPhase2) {
            ...
            console2.log(
                string.concat("[19-21] Repo matured: Bank A repaid $", _toUsd(repaid), " (principal + interest)")
            );
            console2.log(
                string.concat(
                    "        Collateral released - ",
                    vm.toString(corpBondToken.balanceOf(bankA)),
                    " corp bonds back to Bank A"
                )
            );
            ...
            _check(
                custodyRegistry.availableQuantity(C.CORP_BOND, bankA) == C.CORP_BOND_QUANTITY,
                "custody fully unencumbered"
            );
            _check(
                custodyRegistry.availableQuantity(C.T_BOND, bankA) == C.T_BOND_QUANTITY, "custody fully unencumbered"
            );
    ...
    /**
     * @notice Runs the Phase 4 automated margin flow for `repoId` holding the
     *         pledged position `positionId`:
     *         price drop -> preview -> evaluateAll (raises call) -> substitution
     *         -> satisfy, then proves the history records.
     */
    function _runMarginAutomation(bytes32 repoId, bytes32 positionId) internal {
        ...
        // ------------------- STEP 13: operator previews ----------------------
        // Phase 4: a non-mutating evaluation shows the shortfall before any call.
        ...
        // ------------------- STEP 14: operator raises the call ----------------
        // Phase 4: evaluateAll drives the automated margin call (as the agent).
